...
test("the setup sheet is modal, focus-contained, and restores focus when closed", async ({
  ...
  await expect(
    dialog.getByRole("button", { name: "Close studio setup", exact: true }),
  ).toBeFocused();
...
test("the single-webcam setup retains every layout, voice mode, and microphone-test control", async ({
  page,
}) => {
  await page.getByRole("button", { name: /Studio setup/ }).click();
  const dialog = page.getByRole("dialog", { name: "Studio setup" });

  const sectionTabs = dialog.getByRole("tablist", {
    name: "Studio setup sections",
  });
  const sourcesTab = sectionTabs.getByRole("tab", {
    name: "Sources",
    exact: true,
  });
  const layoutTab = sectionTabs.getByRole("tab", {
    name: "Layout",
    exact: true,
  });
  ...
  await expect(
    dialog.getByRole("combobox", { name: "One webcam", exact: true }),
  ).toHaveCount(1);
  await expect(dialog.getByRole("combobox", { name: /webcam/i })).toHaveCount(
    1,
  );
  await expect(
    dialog.getByRole("button", { name: "Enable webcam", exact: true }),
  ).toBeVisible();
  await expect(
    dialog.getByRole("combobox", { name: "Microphone", exact: true }),
  ).toHaveCount(1);
  await expect(
    dialog.getByRole("button", { name: "Enable microphone", exact: true }),
  ).toBeVisible();
  ...
  for (const name of layoutNames) {
    await expect(
      layouts.getByRole("radio", {
        name: new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`),
      }),
    ).toHaveCount(1);
  }
  await expect(
    layouts.getByRole("radio", { name: /^Screen only\b/ }),
  ).toBeChecked();
  await dialog.getByText("Picture-in-picture", { exact: true }).click();
  await expect(
    layouts.getByRole("radio", { name: /^Picture-in-picture\b/ }),
  ).toBeChecked();
  ...
  await expect(voiceModes.getByRole("radio")).toHaveCount(
    voiceModeNames.length,
  );
  for (const name of voiceModeNames) {
    await expect(
      voiceModes.getByRole("radio", { name: new RegExp(`^${name}\\b`) }),
    ).toHaveCount(1);
  }
  await expect(
    voiceModes.getByRole("radio", { name: /^Voice boost\b/ }),
  ).toBeChecked();
  await dialog.getByText("Warm", { exact: true }).click();
  await expect(
    voiceModes.getByRole("radio", { name: /^Warm\b/ }),
  ).toBeChecked();

  await expect(
    dialog.getByRole("heading", { name: "Test your microphone", exact: true }),
  ).toBeVisible();
  await expect(dialog.getByText("Warm", { exact: true }).last()).toBeVisible();
  await expect(
    dialog.getByText(/enable a microphone to record a test sample/i),
  ).toBeVisible();
  await expect(
    dialog.getByRole("button", { name: "Record sample", exact: true }),
  ).toBeDisabled();
});

test("phone, QR, remote-camera, and multi-webcam controls are absent", async ({
  page,
}) => {
  for (const route of ["/", "/studio", "/recordings"]) {
    ...
    expect(visibleText).not.toMatch(
      /\b(?:connect|pair)(?:ed|ing)? (?:a |your )?phone\b/i,
    );
    expect(visibleText).not.toMatch(/\bphone (?:camera|connection|pairing)\b/i);
    expect(visibleText).not.toMatch(
      /\b(?:multiple\s+|multi[- ]?)(?:camera|webcam)s?\b/i,
    );
    expect(visibleText).not.toMatch(/\bremote[- ](?:camera|webcam)s?\b/i);
    expect(visibleText).not.toMatch(
      /\badd (?:another|a second|an additional) (?:camera|webcam)\b/i,
    );

    await expect(
      page.locator(
        '[href*="phone" i], [aria-label*="QR" i], [data-testid*="qr" i]',
      ),
    ).toHaveCount(0);
  }
  ...
  await expect(
    dialog.getByRole("button", { name: /add.*(?:camera|webcam)/i }),
  ).toHaveCount(0);
});

test("the microphone test remains available without gating recording readiness", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name === "edge-mobile",
    "The media pipeline is exercised once in desktop Edge.",
  );
  ...
  const reopenedRecordSample = dialog.getByRole("button", {
    name: "Record sample",
    exact: true,
  });
  await expect(reopenedRecordSample).toBeEnabled();
  await reopenedRecordSample.click();
  const stopSample = dialog.getByRole("button", {
    name: "Stop sample",
    exact: true,
  });
  ...
  await expect(
    dialog.getByText(/This voice mode has been recorded and played back/),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(/Record and play a Natural microphone test first\./),
  ).toHaveCount(0);
});
